...
// Убедись, что картинка фона лежит именно по этому пути:
...
export function Layout() {
  ...
  return (
    <div className="min-h-screen text-slate-200 font-serif relative">
      {/* --- 1. ГЛОБАЛЬНЫЙ ФОН (FIXED) --- */}
      {/* Он приклеен к экрану и стоит на месте при скролле */}
      <div className="fixed inset-0 z-0">
        ...
        />
        {/* Затемнение фона (85%), чтобы текст читался */}
        <div className="absolute inset-0 bg-slate-950/85"></div>
      </div>

      {/* --- 2. КОНТЕНТ САЙТА (Поверх фона) --- */}
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        {/* MAIN (САМОЕ ВАЖНОЕ МЕСТО) */}
        {/* flex-grow заставляет этот блок растянуться и прижать подвал к низу */}
        <main className="flex-grow">
          {/* <Outlet /> - это "дырка", куда вставляется Home, Gallery и т.д. */}
          {/* Если убрать эту строку, страницы не будут видны! */}
          <Outlet />
        </main>
